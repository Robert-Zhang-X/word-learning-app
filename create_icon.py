...
def create_icon():
        ...
        for dx in range(-130, 131):
            ...
            if 0 <= sx < size and 0 <= sy < size:
                if img.getpixel((sx, sy))[3] > 0:
                    ...
                    img.putpixel((sx, sy), (p[0], p[1], p[2], max(p[3], alpha)))
    ...
    # Draw W letter
    w_points = [
        (bubble_x - 25, bubble_y - 25),
        (bubble_x - 15, bubble_y + 20),
        (bubble_x, bubble_y - 5),
        (bubble_x + 15, bubble_y + 20),
        (bubble_x + 25, bubble_y - 25),
    ]
    
    # Simple W using rectangles
    for i, (wx, wy) in enumerate(w_points):
        w_width = 8 if i in [0, 4] else 6
        w_height = 45 if i in [0, 4] else 30
        
        for dy in range(w_height):
            for dx in range(w_width):
                px, py = wx + dx, wy + dy
                if 0 <= px < size and 0 <= py < size:
                    img.putpixel((px, py), purple1)
    ...
    for letter, lx, ly in letters:
        ...
        for dy in range(-12, 13):
            for dx in range(-8, 9):
                px, py = lx + dx, ly + dy
                if 0 <= px < size and 0 <= py < size:
                    if abs(dx) + abs(dy) < 12:  # Simple circle
                        p = img.getpixel((px, py))
                        img.putpixel((px, py), (255, 255, 255, max(p[3], 220)))
    ...
    for y in range(size):
        for x in range(size):
            p = img.getpixel((x, y))
            if p[3] > 0:
                # Add slight darkening at edges
                cx, cy = x - size//2, y - size//2
                dist = math.sqrt(cx*cx + cy*cy) / (size//2)
                if dist > 0.7:
                    darken = int(20 * (dist - 0.7) / 0.3)
